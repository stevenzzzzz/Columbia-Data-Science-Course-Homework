...
setwd("/Users/zhongming/Downloads")
...
# You'll want to type your response to question B(3) here.  Your response should look like:
 resample_data <- diamonds[resample1,]
 resample_data=data.frame(carat=unlist(resample_data[[1]]),price=unlist(resample_data[[2]]))
 resample1_ests <- lm(resample_data$price~resample_data$carat)
 resample1_ests
```
...
# You'll want to type your response to question B(4) here.  Your response should look like:
 ...
 for (b in 1:B) {
   resampled_ests[b,] <- coefficients(lm(diamonds[resampled_values[b,],]$price~diamonds[resampled_values[b,],]$carat))
 }
 head(resampled_ests)
```
...
hist(resampled_ests[,2],main="Slope_Est")
```
...
# You'll want to type your response to question C(1) here.  Your response should look like:
 Cl <- quantile(diff_estimates,probs=0.025)+coefficients(lm0)[2]
 Cu <- quantile(diff_estimates,probs=0.975)+coefficients(lm0)[2]
 int <- c(Cl, Cu)
 int
```
...
```{r}
quantile(resampled_ests[,2],0.025)
quantile(resampled_ests[,2],0.975)
```
